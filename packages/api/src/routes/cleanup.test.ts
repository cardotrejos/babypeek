...
function createCleanupApp(options: {
  ...
  app.post("/", async (c) => {
    ...
    const R2ServiceMock = Layer.succeed(R2Service, {
      ...
      generatePresignedDownloadUrl: () => Effect.succeed({ url: "", key: "", expiresAt: new Date() }),
    ...
    const resultEither = await Effect.runPromise(
      Effect.either(
        program.pipe(
          Effect.provide(CleanupServiceMock),
    ...
    if (resultEither._tag === "Left") {
      return c.json({
        ...
        error: { 
          code: "CLEANUP_FAILED", 
          message: resultEither.left instanceof Error ? resultEither.left.message : "Cleanup failed"
        },
        durationMs,
      }, 500)
    }
...
describe("POST /api/cron/cleanup", () => {
  ...
  describe("Authentication", () => {
    ...
    it("accepts valid x-cron-secret header", async () => {
      const app = createCleanupApp({ cleanupResult: { processed: 0, deleted: 0, failed: 0, errors: [] } })
