...
app.post("/", async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          code: "INVALID_BODY",
          message: `Invalid reason. Must be one of: ${preferenceReasonValues.join(", ")}`,
        },
      },
      400,
    );
  ...
  if ("error" in data) {
    ...
    return c.json({ success: false, error: { code: data.error, message: data.message } }, status);
  }
...
app.get("/stats", async (c) => {
  const getStats = Effect.gen(function* () {
    // Get all preferences
    const allPreferences = yield* Effect.promise(() => db.query.preferences.findMany());
