    ...
    const StripeServiceMock = Layer.succeed(StripeService, {
      ...
      constructWebhookEvent: (_payload: string, _sig: string) => {
        if (options.shouldThrowOnConstruct) {
          return Effect.fail(new PaymentError({ cause: "STRIPE_ERROR", message: "Unexpected error" }))
        }
        if (!options.signatureValid) {
          return Effect.fail(new PaymentError({ cause: "WEBHOOK_INVALID", message: "Invalid signature" }))
        }
...
describe("PostHog Analytics (AC-4)", () => {
  ...
  it("handles null payment_intent gracefully", () => {
    ...
    const paymentIntentId = typeof sessionWithNullPaymentIntent.payment_intent === "string" 
  ...
  it("extracts payment_intent when it is a string", () => {
    ...
    const paymentIntentId = typeof sessionWithPaymentIntent.payment_intent === "string" 
