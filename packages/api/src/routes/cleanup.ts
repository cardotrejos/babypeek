...
app.post("/", async (c) => {
  ...
  const resultEither = await Effect.runPromise(
    Effect.either(program.pipe(Effect.provide(CleanupServiceLive), Effect.provide(R2ServiceLive))),
  );
  ...
  if (result.errors.length > 0) {
    captureException(new Error(`Cleanup partial failure: ${result.errors.length} errors`), {
      context: "cleanup_cron",
