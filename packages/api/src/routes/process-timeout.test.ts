...
describe("Effect.timeout behavior", () => {
  it("should timeout an effect after specified duration", async () => {
    ...
    const timedOut = slowEffect.pipe(Effect.timeout(Duration.millis(50)), Effect.either);
  ...
  it("should succeed if effect completes before timeout", async () => {
    ...
    const timedOut = fastEffect.pipe(Effect.timeout(Duration.millis(1000)), Effect.either);
