  ...
  app.get("/:uploadId", async (c) => {
    ...
    const R2ServiceMock = Layer.succeed(R2Service, {
      ...
      generatePresignedDownloadUrl: () => {
        if (options.r2Error) {
          return Effect.fail(
            new R2Error({ cause: "PRESIGN_FAILED", message: "Failed to generate URL" }),
          );
        }
    ...
    const PurchaseServiceMock = Layer.succeed(PurchaseService, {
      ...
      hasPurchased: () =>
        Effect.succeed(mockPurchase !== null && mockPurchase.status === "completed"),
  ...
  describe("Successful Download (AC-1, AC-2, AC-3)", () => {
    it("returns signed URL when authorized and purchased", async () => {
      ...
      expect(body.downloadUrl).toContain(
        "https://bucket.r2.cloudflarestorage.com/results/result-123/full.jpg",
      );
...
describe("GET /api/download/:uploadId/status (Story 7.5)", () => {
  ...
  function createStatusApp(options: { upload?: Upload | undefined; purchase?: Purchase | null }) {
