...
app.get("/:uploadId", async (c) => {
  ...
  if (resultEither._tag === "Left") {
      ...
      return c.json(
        {
          ...
          error: {
            code: "NOT_FOUND",
            message: "We couldn't find your result. Try uploading again?",
          },
        },
        404,
      );
    ...
    if (error._tag === "R2Error") {
      ...
      captureException(new Error(`R2 download error: ${error.message}`), {
        uploadId,
        cause: error.cause,
      });
      return c.json(
...
app.get("/:uploadId/status", async (c) => {
  ...
  const result = await Effect.runPromise(
    program.pipe(Effect.provide(PurchaseServiceLive), Effect.provide(StripeServiceLive)),
  );
