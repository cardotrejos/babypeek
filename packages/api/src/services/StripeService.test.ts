  ...
  describe("Configuration verification via mock layer", () => {
    ...
    const createCapturingMockLayer = () => {
      ...
      return Layer.succeed(StripeService, {
        ...
        constructWebhookEvent: () => Effect.fail(new PaymentError({ cause: "WEBHOOK_INVALID", message: "Mock" })),
        retrieveSession: () => Effect.fail(new PaymentError({ cause: "STRIPE_ERROR", message: "Mock" })),
      })
    }
