...
const cleanupSingleUpload = (
  ...
  Effect.gen(function* () {
    ...
    const uploadPurchases = yield* Effect.tryPromise({
      try: () => db.select({ id: purchases.id })
        .from(purchases)
        .where(eq(purchases.uploadId, uploadId)),
      catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to query purchases: ${e}`, uploadId }),
    })

    for (const purchase of uploadPurchases) {
      yield* Effect.tryPromise({
        try: () => db.delete(downloads).where(eq(downloads.purchaseId, purchase.id)),
        catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to delete downloads: ${e}`, uploadId }),
      })
    }
    ...
    yield* Effect.tryPromise({
      try: () => db.update(purchases)
        .set({ giftRecipientEmail: GDPR_ANONYMIZED_EMAIL })
        .where(eq(purchases.uploadId, uploadId)),
      catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to anonymize purchase: ${e}`, uploadId }),
    })
    ...
    yield* Effect.tryPromise({
      ...
      catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to delete purchases: ${e}`, uploadId }),
    })

    // 5. Delete upload record
    yield* Effect.tryPromise({
      try: () => db.delete(uploads).where(eq(uploads.id, uploadId)),
      catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to delete upload: ${e}`, uploadId }),
    })
...
const runCleanup: Effect.Effect<CleanupResult, CleanupError, R2Service> = Effect.gen(function* () {
  ...
  const potentiallyStaleUploads = yield* Effect.tryPromise({
    ...
    catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to query stale uploads: ${e}` }),
  })
  ...
  for (const upload of potentiallyStaleUploads) {
    ...
    const purchaseResult = yield* Effect.tryPromise({
      try: () => db.select({ createdAt: purchases.createdAt })
        .from(purchases)
        .where(eq(purchases.uploadId, upload.id))
        .limit(1),
      catch: (e) => new CleanupError({ cause: "DB_FAILED", message: `Failed to query purchase for upload ${upload.id}: ${e}`, uploadId: upload.id }),
    })
