...
describe("geminiRetrySchedule", () => {
  it("should retry up to 3 times (4 total attempts) with predicate", async () => {
    ...
    const program = failingEffect.pipe(
      Effect.retry(geminiRetryScheduleTest.pipe(Schedule.whileInput(isRetryableGeminiError))),
      Effect.either,
    );
  ...
  it("should NOT retry on INVALID_IMAGE (non-retryable)", async () => {
    ...
    const nonRetryableEffect = Effect.gen(function* () {
      attempts++;
      return yield* Effect.fail(new GeminiError({ cause: "INVALID_IMAGE", message: "Bad image" }));
    });
  ...
  it("should NOT retry on CONTENT_POLICY (non-retryable)", async () => {
    ...
    const nonRetryableEffect = Effect.gen(function* () {
      attempts++;
      return yield* Effect.fail(new GeminiError({ cause: "CONTENT_POLICY", message: "Blocked" }));
    });
  ...
  it("should retry on TIMEOUT errors", async () => {
    ...
    const timeoutEffect = Effect.gen(function* () {
      attempts++;
      if (attempts < 2) {
        return yield* Effect.fail(new GeminiError({ cause: "TIMEOUT", message: "Timed out" }));
      }
      return "recovered";
    });
...
describe("geminiRetryScheduleTest", () => {
  it("should stop after 3 retries (4 total attempts)", async () => {
    ...
    const failingEffect = Effect.gen(function* () {
      attempts++;
      return yield* Effect.fail(new GeminiError({ cause: "API_ERROR", message: "always fail" }));
    });

    const program = failingEffect.pipe(Effect.retry(geminiRetryScheduleTest), Effect.either);
