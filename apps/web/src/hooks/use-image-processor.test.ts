...
vi.mock("browser-image-compression", () => ({
  default: (file: File, options: { onProgress?: (p: number) => void }) =>
    mockImageCompression(file, options),
}));
...
describe("useImageProcessor", () => {
  beforeEach(() => {
    ...
    mockHeic2any.mockResolvedValue(new Blob(["mock jpeg content"], { type: "image/jpeg" }));
  ...
  describe("HEIC conversion", () => {
    ...
    it("sets isProcessing to true during conversion", async () => {
      ...
      mockHeic2any.mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve(new Blob(["jpeg"], { type: "image/jpeg" })), 50),
          ),
      );
  ...
  describe("compression", () => {
    ...
    it("calls onProgress during compression", async () => {
      ...
      mockImageCompression.mockImplementation(
        async (file: File, options: { onProgress?: (p: number) => void }) => {
