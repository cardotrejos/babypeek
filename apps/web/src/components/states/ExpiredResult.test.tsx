...
describe("ExpiredResult", () => {
  ...
  describe("rendering", () => {
    it("displays warm expired message", () => {
      ...
      expect(
        screen.getByText("This photo has moved on")
      ).toBeInTheDocument()
      expect(
        screen.getByText(/automatically deleted after 30 days/i)
      ).toBeInTheDocument()
    })
    ...
    it("displays privacy explanation", () => {
      ...
      expect(
        screen.getByText(/Why we do this:/i)
      ).toBeInTheDocument()
      expect(
        screen.getByText(/ultrasound images are personal/i)
      ).toBeInTheDocument()
    })
    ...
    it("displays create new CTA button", () => {
      ...
      expect(
        screen.getByRole("button", { name: /Create a New Portrait/i })
      ).toBeInTheDocument()
    })
    ...
    it("displays time estimate", () => {
      ...
      expect(
        screen.getByText(/It only takes 90 seconds/i)
      ).toBeInTheDocument()
    })
  ...
  describe("analytics", () => {
    ...
    it("only tracks analytics once", () => {
      const { rerender } = render(
        <ExpiredResult resultId="test-123" source="result" />
      )
