...
describe("GiftCheckoutButton - Story 6.7", () => {
  ...
  describe("Analytics (AC-6)", () => {
    ...
    it("tracks gift_checkout_created on successful checkout", async () => {
      ...
      await waitFor(() => {
        expect(posthog.capture).toHaveBeenCalledWith("gift_checkout_created", expect.objectContaining({
          share_id: "share_123",
          upload_id: "upload_123",
        }))
      })
    ...
    it("tracks gift_checkout_error on failure", async () => {
      ...
      await waitFor(() => {
        expect(posthog.capture).toHaveBeenCalledWith("gift_checkout_error", expect.objectContaining({
          share_id: "share_123",
        }))
      })
    })
