...
describe("EmailInput", () => {
  ...
  it("shows error for empty email on blur", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("alert")).toHaveTextContent(/we'll need your email/i);
    });
  });
  ...
  it("shows error for invalid email format on blur", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("alert")).toHaveTextContent(/doesn't look quite right/i);
    });
  });
  ...
  it("calls onValidityChange with true for valid email", async () => {
    ...
    render(
      <EmailInput {...defaultProps} value="test@example.com" onValidityChange={onValidityChange} />,
    );
  ...
  it("calls onValidityChange with false for invalid email", async () => {
    const user = userEvent.setup();
    const onValidityChange = vi.fn();
    render(<EmailInput {...defaultProps} value="invalid" onValidityChange={onValidityChange} />);
  ...
  it("displays external error over internal validation", () => {
    render(<EmailInput {...defaultProps} value="test@example.com" error="Server error" />);
