...
describe("ProcessingScreen", () => {
  ...
  describe("Stage Indicator", () => {
    ...
    it("maps generating stage to Creating (step 2)", () => {
      render(
        <ProcessingScreen stage="generating" progress={40} isComplete={false} isFailed={false} />,
      );

      expect(screen.getByText("Creating your baby's portrait...")).toBeInTheDocument();
    });

    it("maps storing stage to Finishing (step 3)", () => {
      render(
        <ProcessingScreen stage="storing" progress={70} isComplete={false} isFailed={false} />,
      );
    ...
    it("maps watermarking stage to Finishing (step 3)", () => {
      render(
        <ProcessingScreen stage="watermarking" progress={85} isComplete={false} isFailed={false} />,
      );
    ...
    it("maps complete stage to Finishing (step 3)", () => {
      render(
        <ProcessingScreen stage="complete" progress={100} isComplete={true} isFailed={false} />,
      );
    ...
    it("defaults to Analyzing when stage is null", () => {
      render(<ProcessingScreen stage={null} progress={0} isComplete={false} isFailed={false} />);
  ...
  describe("Progress Bar", () => {
    it("renders progress bar with correct value", () => {
      render(
        <ProcessingScreen stage="generating" progress={45} isComplete={false} isFailed={false} />,
      );
    ...
    it("includes aria-label with progress and stage info", () => {
      render(
        <ProcessingScreen stage="validating" progress={25} isComplete={false} isFailed={false} />,
      );

      const progressBar = screen.getByRole("progressbar");
      expect(progressBar).toHaveAttribute("aria-label", "Processing 25% complete, stage 1 of 3");
    });

    it("displays progress percentage text", () => {
      render(
        <ProcessingScreen stage="generating" progress={60} isComplete={false} isFailed={false} />,
      );
  ...
  describe("Accessibility (AC-6)", () => {
    ...
    it("announces stage changes to screen readers", () => {
      ...
      const liveRegion = container.querySelector('[aria-live="polite"]');
      expect(liveRegion).toHaveTextContent("Stage 1 of 3: Analyzing your ultrasound");
      ...
      expect(liveRegion).toHaveTextContent("Stage 2 of 3: Creating your baby's portrait");
    });
  ...
  describe("Baby Facts", () => {
    ...
    it("rotates facts every 10 seconds", async () => {
      ...
      expect(screen.getByText("Babies start dreaming before they're born.")).toBeInTheDocument();
    });
  });
  ...
  describe("Image Skeleton", () => {
    it("renders skeleton placeholder for image area", () => {
      ...
      expect(screen.getByText("Your portrait will appear here")).toBeInTheDocument();
    });
  });
  ...
  describe("Stage Copy", () => {
    it("shows stage-specific main copy", () => {
      ...
      rerender(
        <ProcessingScreen stage="watermarking" progress={90} isComplete={false} isFailed={false} />,
      );
